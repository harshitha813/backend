...
const newuser=db.newusertabs;
exports.create=(req,res)=>{
    if(!req.body.name){
        res.status(400).send({message:"Content can not be empty"});
        return;
    }
    const newusertab=new newuser({
        name:req.body.name,
...
exports.findOne=(req,res) => {
  ...
  exports.update=(req,res) =>{
      ...
      const id=req.params.id;
      newuser.findByIdAndUpdate(id,req.body,{ useFindAndModify:false})
      .then(data =>{
          if(!data){
  ...
  exports.delete=(req,res)=>{
      const id=req.params.id;
      newuser.findByIdAndRemove(id,{useFindAndModify:false})
      .then(data =>{
          if(!data){
              res.status(404).send({
